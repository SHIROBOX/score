...
public class HsubAssessmentServiceImpl implements AssessmentService {
    ...
    public int calculate(int count3, int count2, int count1, int count0, int count8) {
        ...
        BigDecimal percentOfAnswer23 = (BigDecimal.valueOf(count3 + count2).divide(new BigDecimal(totalQustion), 2, RoundingMode.HALF_UP)).multiply(new BigDecimal(100));
        BigDecimal percentOfAnswer0 = (BigDecimal.valueOf(count0).divide(new BigDecimal(totalQustion), 2, RoundingMode.HALF_UP)).multiply(new BigDecimal(100));
        int gruopResult;
        if (count0 > 0) {
